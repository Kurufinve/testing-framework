...
for model_name in models:
    ...
    for test_name in tests:
        ...
        try:
            ...
            continue
        try:
            (stable_mu_extrema, full_mu_range) = mu_range(cur_min_EV, cur_composition,
                                                          data[model_name][test_name]["bulk_struct_test"],
        ...
        for defect_label in data[model_name][test_name]["defects"]:
            ...
            if 'dmu' in defect:
                ...
                mu_Z = defect['dmu'][1]
                if len(stable_mu_extrema) == 0:
                    print("DEFECT", model_name, test_name, ind, Z, "(Ef0 =", Ef0, ", Ef = ", Ef,
                          ") but no stable mu range exists")
                else:
                    ...
                    print("Ef0 ", Ef0, " + ( mu_{} = [".format(mu_Z), mu_min, "--", mu_max, "] ) = [", Ef0 + mu_min,
                          "--", Ef0 + mu_max, "]")
                    print("Ef ", Ef, " + ( mu_{} = [".format(mu_Z), mu_min, "--", mu_max, "] ) = [", Ef + mu_min, "--",
                          Ef + mu_max, "]")
                    table_entry.append((test_name, ind, Z,
                                        "{} + ( mu_{} = [ {} -- {} ] ) = [ {} -- {} ]".format(Ef0, mu_Z, mu_min, mu_max,
                                                                                              Ef0 + mu_min,
                                                                                              Ef0 + mu_max),
                                        "{} + ( mu_{} = [ {} -- {} ] ) = [ {} -- {} ]".format(Ef, mu_Z, mu_min, mu_max,
                                                                                              Ef + mu_min,
                                                                                              Ef + mu_max)))
            else:
...
for model_name in models:
    ...
    for d in defects_sorted:
        if model_name in defect_table_data[l]:
            print(" & " + "{}".format(defect_table_data[d][model_name][0]) + " & " + "{}".format(
                defect_table_data[d][model_name][1]), end='')
        else:
            print(" & & ", end='')
print("\n\\end{tabular}")
