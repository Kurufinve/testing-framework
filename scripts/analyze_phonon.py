...
for model_name in models:
    ...
    for (bulk_i, bulk_struct_test) in enumerate(bulk_struct_tests):
        ...
        if ref_model_data is not None and 'DOS' in ref_model_data:
            ax_DOS.plot(ref_model_data['DOS']['freq'], ref_model_data['DOS']['val'], ":", color='C{}'.format(bulk_i),
                        label=ref_model_name)
        if 'DOS' in model_data:
            ax_DOS.plot(model_data['DOS']['freq'], model_data['DOS']['val'], "-", color='C{}'.format(bulk_i),
                        label=bulk_struct_test)
