...
parser = argparse.ArgumentParser(
    description='Run all tests on all models.  Default command line options in dict of lists in "default_run_opts.json", keys are "global" or regexps matching model names')
...
parser.add_argument('--base_model', '-B', action='store', type=str,
                    help='model to use as initial config for tests where it is enabled')
parser.add_argument('--models_path', '-P', action='store', type=str, help='path to models directory',
...
if args.models is not None:
    ...
    models = list(
        itertools.chain.from_iterable([glob.glob(os.path.join(args.models_path, d, 'model.py')) for d in args.models]))
else:
...
if args.omit_tests is not None:
    for omit_test in list(
            itertools.chain.from_iterable([glob.glob(os.path.join(tests_path, d)) for d in args.omit_tests])):
...
for model in models:
    ...
    for test in tests:
        ...
        cmd_args = '{0} {1} {2} --test_set {3}'.format(run_model_test,
                                                       "'" + os.path.join(args.models_path, model_name) + "'",
                                                       test_name, args.test_set)
        if force:
            cmd_args += ' -force'
        ...
        if args.bugs:
            ...
            cmd = ('env REDIRECT_IO="' + cmd_args + '" bugs -exec=python ' + mpi_cmd + ' -time=96h -np=' + (
                        '%d' % np) + ' -script=' + bugs_script +
