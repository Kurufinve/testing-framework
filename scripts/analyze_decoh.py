...
for test in tests:
    ref_opening = decoh_data[ref_model_name][test]['surface_decohesion_unrelaxed_opening']
    ...
    for model in sorted(decoh_data.keys()):
        if bool(decoh_data[model]) is True and model != ref_model_name:
            i += 1
            ax1.plot(decoh_data[model][test]['surface_decohesion_unrelaxed_opening'],
                     decoh_data[model][test]['surface_decohesion_unrelaxed_energy'], label=model,
                     color=struct_colors[i])  # , linestyle=other_linestyles[0])
            ax2.plot(decoh_data[model][test]['surface_decohesion_unrelaxed_opening'],
                     decoh_data[model][test]['surface_decohesion_unrelaxed_stress'], label=model,
                     color=struct_colors[i])  # , linestyle=other_linestyles[0])
