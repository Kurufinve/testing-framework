...
def get_multicomponent_constraints(test_set, models, multicomponent_constraints_structs):
    ...
    energy_data = {}
    if debug:
        print("get_multicomponent_constraints", multicomponent_constraints_structs,
              isinstance(multicomponent_constraints_structs, basestring))
    ...
    for model_name in models:
        ...
        if isinstance(multicomponent_constraints_structs, basestring):
            ...
            struct_name_list = glob.glob(
                "{}-model-{}-test-{}-properties.json".format(test_set, model_name, multicomponent_constraints_structs))
            struct_name_list = [
                x.replace("{}-model-{}-test-".format(test_set, model_name), "").replace("-properties.json", "") for x in
                struct_name_list]
        else:
...
def analyze_start(default_tests=['*']):
    ...
    parser.add_argument('--tests', '-t', action='store', nargs='+', type=str, help='tests to include (globs)',
    ...
    parser.add_argument('--models_path', '-P', action='store', help='path to models directory',
                        default=os.path.join(os.getcwd(), '../models'))
    parser.add_argument('--REF_E_offset', type=float, action='store', help='offset of reference model energy/atom',
                        default=0.0)
    ...
    models = [os.path.split(f)[1] for f in
              list(itertools.chain.from_iterable([glob.glob(os.path.join(args.models_path, d)) for d in args.models]))]
    my_path = os.path.dirname(os.path.realpath(__file__))
    tests_path = os.path.join(my_path, "..", "tests", args.test_set)
    tests = [os.path.split(f)[1] for f in
             list(itertools.chain.from_iterable([glob.glob(os.path.join(tests_path, d)) for d in args.tests]))]
