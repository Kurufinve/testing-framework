...
(args, models, tests, default_analysis_settings) = analyze_start('gap_6_rss_like_open_ended_minima_rerelax_sd2')
...
# ace_name = "pACE_B8_N4_18_rid_1.20"

A, B = rss_data[ref_model_name]['gap_6_rss_like_open_ended_minima_rerelax_sd2']["V_unrelaxed"], \
       rss_data[ref_model_name]['gap_6_rss_like_open_ended_minima_rerelax_sd2']["E_unrelaxed"]
C, D = rss_data[ref_model_name]['gap_6_rss_like_open_ended_minima_rerelax_sd2']["V_relaxed"], \
       rss_data[ref_model_name]['gap_6_rss_like_open_ended_minima_rerelax_sd2']["E_relaxed"]
...
for (i, model) in enumerate(models):
    if model != ref_model_name:
        C, D = rss_data[model]['gap_6_rss_like_open_ended_minima_rerelax_sd2']["V_relaxed"], \
               rss_data[model]['gap_6_rss_like_open_ended_minima_rerelax_sd2']["E_relaxed"]
        L = list(zip(list(zip(A, B)), list(zip(C, D))))
        lc = LineCollection(L, colors=["grey"], linewidth=0.5)
        a0.add_collection(lc)
